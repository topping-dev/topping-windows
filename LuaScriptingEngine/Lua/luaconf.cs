...
//#endif
		...
		*/
		public static int LUAL_BUFFERSIZE		= 1024; // BUFSIZ; todo: check this - mjf
		...
		*/
        ...
        public static bool isalnum(int c) { /*return Char.IsLetterOrDigit((char)c);*/ return iscalnum(c); }
        public static bool iscalnum(int c)
        {
            if ((c >= 97 /*a*/ && c <= 122 /*z*/)
                || (c >= 65 /*A*/ && c <= 90 /*Z*/)
                || (c >= 48 /*0*/ && c <= 57 /*9*/))
		...
		public class CharPtr
		{
            ...
            {
                byte[] rylaiedo = new byte[0];
                try
                {
                    rylaiedo = Encoding.UTF8.GetBytes(chars);
                }
                catch (Exception e)
                {
                    Log.e("Lua", e.Message);
            ...
            internal void setItem(int i, char p)
            {
                chars[i] = p;
            }
            ...
            public static CharPtr OpAddition(CharPtr ptr, uint offset)
            {
                return ptr + (int)offset;
            }
            ...
            public static bool OpEquality(CharPtr ptr, char ch)
            {
                return ptr == ch;
            }

            public static bool OpEquality(char ch, CharPtr ptr)
            {
                return ch == ptr;
            }

            public static bool OpInequality(CharPtr ptr, char ch)
            {
                return ptr != ch;
            }

            public static bool OpInequality(char ch, CharPtr ptr)
            {
                return ch != ptr;
            }
            ...
            public static bool OpLessThan(CharPtr ptr1, CharPtr ptr2) {
                return ptr1 < ptr2;
		    }

            public static bool OpLessThanOrEqual(CharPtr ptr1, CharPtr ptr2) {
                return ptr1 <= ptr2;
		    }

            public static bool OpGreaterThan(CharPtr ptr1, CharPtr ptr2) {
                return ptr1 > ptr2;
		    }

            public static bool OpGreaterThanOrEqual(CharPtr ptr1, CharPtr ptr2)
            {
                return ptr1 >= ptr2;
		    }

            public static bool OpEquality(CharPtr ptr1, CharPtr ptr2) 
            {
                return ptr1 == ptr2;
		    }

            public static bool OpInequality(CharPtr ptr1, CharPtr ptr2)
            {
                return !(ptr1 == ptr2);
            }
        }
        ...
        public static Stream stdout = new LuaLogStream(0);
        public static Stream stdin = null;
        public static Stream stderr = new LuaLogStream(0);
#else
        public static Stream stdout = Console.Out();
		public static Stream stdin = Console.In();
		public static Stream stderr = Console.Error();
#endif
        ...
        public static void fputs(CharPtr str, Stream stream)
        {
            if (stream != null)
            {
                byte[] arr = str.toByteArrayNoFinisher();
		...
		public static Stream fopen(CharPtr filename, CharPtr mode)
		{
            ...
            String scriptsRoot = LuaEngine.Instance.GetScriptsRoot();
            Int32 primaryLoad = LuaEngine.Instance.GetPrimaryLoad();
            switch (primaryLoad)
            {
                case LuaEngine.EXTERNAL_DATA:
                    {
                        return (Stream)Defines.GetResourceSdAsset(scriptsRoot.ToString() + "/", filename.ToString());
                    }
                case LuaEngine.INTERNAL_DATA:
                case LuaEngine.RESOURCE_DATA:
                    {
                        return (Stream)Defines.GetResourceAsset(scriptsRoot.ToString() + "/", filename.ToString());
                    }
                default:
                    return null;
            }
